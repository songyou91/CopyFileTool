 ...
 */
public class Archives implements Closeable {

	private static Logger logger = Logger.getLogger(Archives.class.getName());
	...
	public Archives(File file, String password, boolean test) throws RarException, IOException {
	 ...
	 * @throws RarException
	 */
	public Archives(File file, UnrarCallback unrarCallback, String password, boolean test)
	...
	void setFile(File file) throws IOException {
		...
		for (BaseBlock block : headers) {
			if (block.getHeaderType() == UnrarHeadertype.FileHeader) {
				totalPackedSize += ((FileHeaders) block).getFullPackSize();
			}
		}
	 ...
	 */
	public List<FileHeaders> getFileHeaders() {
		List<FileHeaders> list = new ArrayList<FileHeaders>();
		for (BaseBlock block : headers) {
			if (block.getHeaderType().equals(UnrarHeadertype.FileHeader)) {
				list.add((FileHeaders) block);
			}
		}
		return list;
	}

	public FileHeaders nextFileHeader() {
		int n = headers.size();
		while (currentHeaderIndex < n) {
			BaseBlock block = headers.get(currentHeaderIndex++);
			if (block.getHeaderType() == UnrarHeadertype.FileHeader) {
				return (FileHeaders) block;
			}
		}
		return null;
	}
		...
		while (!stop) {
			...
			default:
				...
				case FileHeader:
					...
					FileHeaders fh = new FileHeaders(blockHead, fileHeaderBuffer);
					// if (DEBUG) {
					// fh.print();
					// }
					headers.add(fh);
	 ...
	 * @throws RarException
	 */
	public void extractFile(FileHeaders hd, OutputStream os) throws RarException {
	...
	private void doExtractFile(FileHeaders hd, OutputStream os)
			throws RarException, IOException {
		dataIO.init(os);
		dataIO.init(hd);
